...
for audio in list(audio_caps.keys())[507:]:
    print(audio)
    prompt = f'''Video caption: '{video_caps[audio]['pred']}' \nMusic caption: '{audio_caps[audio]['pred']}' \nDescribe the video from the given video and music information. Return answer only without intro/outro.'''
    ...
    x =  open('mixtral_output_video.json')
    z = json.load(x)
    z.update(outputs)
    with open('mixtral_output_video.json', 'w', encoding ='utf8') as x: 
        json.dump(z, x)